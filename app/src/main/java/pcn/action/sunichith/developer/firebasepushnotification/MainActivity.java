...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
        ...
        switch (view.getId()){
            case R.id.sendnotification_btn:
              ...
              //  SendNotifications();
                ...
                //here we send Notification to Topic
                SendNotificationToaubscribeTopic();
                break;
            case R.id.subscrribr:
                FirebaseMessaging.getInstance().subscribeToTopic("Update1")
                        .addOnCompleteListener(new OnCompleteListener<Void>() {
                            @Override
                            public void onComplete(@androidx.annotation.NonNull Task<Void> task) {
                                if (task.isSuccessful()){
                                    Toast.makeText(MainActivity.this, "Topic Subscribe", Toast.LENGTH_SHORT).show();
                                }else {
                                    Toast.makeText(MainActivity.this, "Topic not  Subscribe", Toast.LENGTH_SHORT).show();
                                }
                            }
                        });
                break;
            case R.id.Unsubscrribr:
                FirebaseMessaging.getInstance().unsubscribeFromTopic("Update1").addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
                    public void onComplete(@androidx.annotation.NonNull Task<Void> task) {
                        if (task.isSuccessful()){
                            Toast.makeText(MainActivity.this, "Topic UnSubscribe", Toast.LENGTH_SHORT).show();
                        }else {
                            Toast.makeText(MainActivity.this, "Topic not  UnSubscribe", Toast.LENGTH_SHORT).show();
                        }
                    }
                });
                break;
    ...
    private void SendNotificationToaubscribeTopic() {
        updateApi.sendNotification23(new MainObj(new MessageObject("/topics/Update1",new NotificationObj("Sunishchit Developer","djhb08978")))).enqueue(new Callback<MyResponse>() {
            @Override
